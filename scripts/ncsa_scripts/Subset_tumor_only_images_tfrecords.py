...
def argument_parse():
	...
	parser.add_argument("-l","--label",help="predicted image labels dir",required="True")
...
def main():	
	...
	print("Entered INPUT label Filename "+arg.label)
	print("Entered Input TFfile "+arg.input_file)
	print("Entered Output TFfile "+arg.tf_output)

	label_file=arg.label
	tf_input=arg.input_file
	tf_output=arg.tf_output
	'''Reading predicted TCGA tumor label file'''
	fobj = open(label_file)
	'''writing output tfrecord'''
	tf_writer=tf.python_io.TFRecordWriter(tf_output)
	'''Reading the input tf record'''
	for example in tf.python_io.tf_record_iterator(tf_input):
