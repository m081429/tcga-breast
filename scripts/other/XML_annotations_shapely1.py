...
def main():
	...
	for j in range(1,len(poly_regions)):
		print("Region "+str(j))
		coords1=regions[j]
		min=np.argmin(coords1, axis = 0)
		...
		for t in range(0,len(list_min_x)):	
			...
			img_patch = Image.new('L', (full_patch_size_x, full_patch_size_y), 0)
			np_img = np.array(img_patch)
			for c1 in range(0,full_patch_size_x):
				for c2 in range(0,full_patch_size_y):
					np_img=(253,231,36,255)
			print("Sucess")
			num_patch="_X_"+str(min_x)+"_"+str(max_x)+"_Y_"+str(min_y)+"_"+str(max_y)
			# # Create figure and axes
			fig,ax = plt.subplots()
			ax.imshow(np_img)
			ax.set_xlim(0, full_patch_size_x)
			ax.set_ylim(full_patch_size_y, 0)
			for s in range(0,len(poly_regions)):
				...
				if len(poly1_temp) > 0:
					try:
						...
						patch = PolygonPatch(poly2, facecolor=[0,0,0], edgecolor=[0,0.5,0], alpha=0.7, zorder=2)
						ax.add_patch(patch)
					except:
						pass
			# patch = PolygonPatch(polygon, facecolor=[0,0,0], edgecolor=[0,0.5,0], alpha=0.7, zorder=2)
			# ax.add_patch(patch)
			plt.savefig(num_patch, alpha=True, dpi=300)
			plt.show()	
		sys.exit(1)
