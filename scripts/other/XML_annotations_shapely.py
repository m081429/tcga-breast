...
def main():
	...
	print("Total Num of region "+str(len(poly_regions)))
	...
	for j in range(0,len(poly_regions)):
		...
		total_min_x=int(coords1[min[0]][0])-patch_size
		total_min_y=int(coords1[min[1]][1])-patch_size
		total_max_x=int(coords1[max[0]][0])+patch_size
		total_max_y=int(coords1[max[1]][1])+patch_size
		total_full_patch_size_x=total_max_x-total_min_x
		total_full_patch_size_y=total_max_y-total_min_y
		total_size=total_full_patch_size_x*total_full_patch_size_y
		#print(total_size)
		#sys.exit(1)
		list_min_x=[]
		list_min_y=[]
		...
		if total_size>1000000000:
			#method=1
			mid_x=int((total_max_x+total_min_x)/2)
			mid_y=int((total_max_y+total_min_y)/2)
			list_min_x.append(total_min_x)
			list_max_x.append(mid_x)
			list_min_y.append(total_min_y)
			list_max_y.append(mid_y)
			
			list_min_x.append(total_min_x)
			list_max_x.append(mid_x)
			list_min_y.append(mid_y+1)
			list_max_y.append(total_max_y)
			
			list_min_x.append(mid_x+1)
			list_max_x.append(total_max_x)
			list_min_y.append(total_min_y)
			list_max_y.append(mid_y)
			...
			list_min_y.append(total_min_y)
			list_max_y.append(total_max_y)
		#print(str(total_size)+' '+str(total_min_x)+' '+str(total_max_x)+' '+str(total_min_y)+' '+str(total_max_y))	
		#print(list_min_x)
		#print(list_min_y)
		#print(list_max_x)
		#print(list_max_y)
		...
		for t in range(0,len(list_min_x)):	
				...
				for s in range(0,len(poly_regions)):
					...
					if len(poly1_temp) > 0:
						try:
							...
							xml_ann_poly_all.append(poly1_temp)
						except:
							pass
			# else:
				# '''Writing the binary mask'''
				# '''creating polygon for a patch'''
				# try:
					# path_poly=geo.box(min_x, min_y, max_x, max_y, ccw=True)
				
					# '''Itirating through all the XML polygons'''
					# for s in range(0,len(poly_regions)):
						# '''intersecting patch with each xml annotation polygon'''
						# poly1=path_poly.intersection(poly_regions[s])
						# '''checking if polygon is empty'''
						# if poly1.length > 0:
							# '''differentiating if it is a polygon or not'''
							# if poly1.geom_type == "MultiPolygon":
								# for sub_poly1 in poly1:
									# poly1_temp=[]
									# for tmp_x,tmp_y in sub_poly1.exterior.coords:
										# tmp_x=int(tmp_x)-min_x
										# tmp_y=int(tmp_y)-min_y
										# tmp_tup=(tmp_x,tmp_y)
										# poly1_temp.append(tmp_tup)
									# try:
										# poly2 = Polygon(poly1_temp)
										# xml_ann_poly_all.append(poly2)
									# except:
										# pass
							# elif poly1.geom_type == "Polygon":
								# poly1_temp=[]
								# for tmp_x,tmp_y in poly1.exterior.coords:
									# tmp_x=int(tmp_x)-min_x
									# tmp_y=int(tmp_y)-min_y
									# tmp_tup=(tmp_x,tmp_y)
									# poly1_temp.append(tmp_tup)
			...
			# mask_bin = Image.new('L', (full_patch_size_x, full_patch_size_y), 0)
			...
			img = Image.new('L', (full_patch_size_x, full_patch_size_y), 0)
			
			#print("sucess 1")
			for i in range(0,len(xml_ann_poly_all)):
				try:
					ImageDraw.Draw(img).polygon(xml_ann_poly_all[i], outline=1, fill=1)
				except:
					pass
					
			#print("sucess 2")		
			mask_bin = np.array(img)
			ax_bin.imshow(mask_bin)
			...
			while start_x+patch_size < max_x:
				...
				while start_y+patch_size < max_y:
					...
					if len(idx[0])>0 and len(idx[1])>0 and width==patch_size and height==patch_size: 
						...
						#t = M1[current_y:current_y+patch_size,current_x:current_x+patch_size,:,]
						t = M1[current_y:current_y+patch_size,current_x:current_x+patch_size]
